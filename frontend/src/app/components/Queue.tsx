...
export function Queue() {
  ...
  const needsReviewCount = posts.filter((post) => post.asset.status === 'PENDING_REVIEW').length;

  const filtered = posts.filter((post) => {
    if (filter === 'All') return true;
    if (filter === 'Needs review') return post.asset.status === 'PENDING_REVIEW';
    return post.platform === filter.toUpperCase();
  });
  ...
  const handleApprove = async (post: ApiPost) => {
    ...
    try {
      ...
      setPosts((previous) =>
        previous.map((item) =>
          item.id === post.id ? { ...item, asset: { ...item.asset, status: 'APPROVED' } } : item
        )
      );
    } finally {
      setApprovingId(null);
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        ...
        <div className="flex items-center gap-2 text-xs text-zinc-500 px-3 py-1.5 bg-white/5 rounded-lg border border-white/10 w-fit">
      ...
      <div className="flex gap-2 flex-wrap">
        {['All', 'YouTube', 'Instagram', 'Needs review'].map((value) => (
          <button
            key={value}
      ...
      {filtered.length === 0 ? (
        <div className="p-10 sm:p-12 text-center text-zinc-500 text-sm border border-white/10 rounded-2xl bg-white/5">
            ...
            return (
              <motion.div
                ...
                className={`bg-gradient-to-r from-zinc-900/60 to-zinc-950/60 backdrop-blur-xl rounded-2xl border transition-all overflow-hidden ${
                  isExpanded
                    ? 'border-white/20 shadow-[0_0_30px_-5px_rgba(255,255,255,0.06)]'
                    : 'border-white/10 hover:border-white/20'
                }`}
              >
                <button onClick={() => setSelected(isExpanded ? null : post.id)} className="w-full p-4 sm:p-5 text-left">
                  <div className="flex flex-col gap-4 lg:flex-row lg:items-center">
                    <div className="flex items-start gap-4 flex-1 min-w-0">
                      <div className={`w-2 h-2 mt-1 rounded-full flex-shrink-0 ${config.dot} ${status === 'live' ? 'animate-pulse' : ''}`} />
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-white font-medium break-words">{post.asset.title}</p>
                      </div>
                    </div>
                    <div className="flex flex-wrap items-center gap-3 flex-shrink-0">
                      <span className={`flex items-center gap-1.5 text-xs font-medium px-2.5 py-1 rounded-md border ${config.color}`}>
                ...
                {isExpanded && (
                  <motion.div
                    ...
                    className="border-t border-white/10 p-4 sm:p-5 space-y-4"
                  >
                    {status === 'processing' ? (
                      ...
                      <>
                        {caption ? (
                          <div>
                            ...
                            <p className="text-sm text-zinc-300 leading-relaxed bg-white/5 rounded-xl p-4 border border-white/10 break-words">
                              {caption}
                            </p>
                          </div>
                        ) : (
                          ...
                          <div>
                            ...
                            <div className="flex flex-wrap gap-2">
                              {hashtags.map((tag, index) => (
                                <span
                                  key={index}
                                  className="text-xs px-2 py-1 bg-violet-500/10 text-violet-400 rounded-md border border-violet-500/20"
                                >
                                  {tag}
                                </span>
                              ))}
